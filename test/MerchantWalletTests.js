...
contract('MerchantWallet', function (accounts) {
    ...
    const BINANCE_ADDRESS = accounts[8]
    const PRICE = 1000
    ...
    it('should allow deposit all tokens to exchange address with transaction', async () => {
        await token.mint(wallet.address, PRICE)

        const min_amount = 100
        await wallet.withdrawAllTokensToExchange(token.address, BINANCE_ADDRESS, min_amount, { from: MERCHANT2 })

        var walletBalance = await token.balanceOf(wallet.address)

        assert.equal(walletBalance.toNumber(), 0)
    })

    it('should allow deposit all tokens to exchange address with transaction by monetha', async () => {
        await token.mint(wallet.address, PRICE)

        const min_amount = 100
        await wallet.withdrawAllTokensToExchange(token.address, BINANCE_ADDRESS, min_amount, { from: PAYMENT_PROCESSOR_CONTRACT })
    ...
    it('should not allow to deposit all tokens with transaction when balance is lower than threshold', async () => {
        const balance = await token.balanceOf(wallet.address)

        return await wallet.withdrawAllToExchange(token.address, BINANCE_ADDRESS, balance.plus(1), { from: PAYMENT_PROCESSOR_CONTRACT }).should.be.rejected
    })

    it('should not allow other addresses to deposit all tokens with transaction', async () => {
        const amount = 0

        return wallet.withdrawAllToExchange(token.address, BINANCE_ADDRESS, amount, { from: MERCHANT }).should.be.rejected
    })
